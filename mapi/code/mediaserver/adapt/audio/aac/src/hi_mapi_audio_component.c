...
static HI_S32 AencCheckAACAttr(const AENC_ATTR_AAC_S* pstAACAttr)
{
    ...
    if (pstAACAttr->enSoundType >= HI_MPP_AUDIO_SOUND_TYPE_BUTT)
    {
        printf("[Func]:%s [Line]:%d [Info]:%s\n",
    ...
    if ((pstAACAttr->enAACType == AAC_TYPE_EAACPLUS) && (pstAACAttr->enSoundType != HI_MPP_AUDIO_SOUND_TYPE_STEREO))
    {
        printf("[Func]:%s [Line]:%d [Info]:%s\n",
               __FUNCTION__, __LINE__, "invalid sound mode for AAC encoder");
        return HI_ERR_MAPI_AENC_ILLEGAL_PARAM;
...
HI_S32 OpenAACEncoder(HI_VOID* pEncoderAttr, HI_VOID** ppEncoder)
{
    ...
    *ppEncoder = (HI_VOID*)pstEncoder;
    ...
    if ((HI_MPP_AUDIO_SOUND_TYPE_MONO == pstAttr->enSoundType)&& AAC_TYPE_EAACPLUS != pstAttr->enAACType)
    {
                    ...
                    HI_U8* pu8Outbuf, HI_U32* pu32OutLen)
{
    ...
    if (HI_MPP_AUDIO_SOUND_TYPE_STEREO == pstEncoder->stAACAttr.enSoundType)
    {
        /* 检查数据是否为立体声模式 */
        if (HI_MPP_AUDIO_SOUND_TYPE_STEREO != pstData->enSoundType)
        {
            printf("[Func]:%s [Line]:%d [Info]:%s\n",
...
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_MPP_ADEC_DECODER_S stAacDec;
    
    if(s32Handle_DecAac != -1)
    {
        printf("[%s][%d] aacDec have inited, return successful \n",__FUNCTION__,__LINE__);
        return HI_SUCCESS;
    }
    printf("+++++++++++++++++++HI_MAPI_ADEC_AacInit+++++++++++\n");
    
#if 1
    stAacDec.enType = HI_MPP_AUDIO_FORMAT_AAC;
    strncpy(stAacDec.aszName, "Aac", 16);
    ...
    s32Ret = HI_MAPI_Register_ExtAudioDecoder(&stAacDec, &s32Handle_DecAac);
    if(HI_SUCCESS != s32Ret)
    {
        printf("HI_MAPI_Register_ExtAudioDecoder return ERR, %x", s32Ret);
        return s32Ret;
    }
    //printf(">>>register AAC ok...., s32Handle_DecAac=%d\n", s32Handle_DecAac);
#endif
...
HI_S32 HI_MAPI_ADEC_AacDeInit(HI_VOID)
{
    ...
    if(s32Handle_DecAac == -1)
    {
        printf("[%s][%d] aacDec have Deinited, return successful \n", __FUNCTION__, __LINE__);
        return HI_SUCCESS;
    }

    s32Ret = HI_MAPI_UnRegister_ExtAudioDecoder(s32Handle_DecAac);
    if (HI_SUCCESS != s32Ret)
    {
        printf("[%s][%d] HI_MAPI_UnRegister_ExtAudioDecoder fail,s32Ret:%d\n", __FUNCTION__, __LINE__, s32Ret);
        return s32Ret;
    }
    s32Handle_DecAac = -1;
...
{
    HI_S32 s32Ret = HI_SUCCESS;
    pthread_mutex_lock(&m_AdecEncoderLock);

    printf("+++++++++++++++++++HI_MAPI_Register_AudioDecoder++++++++++1\n");
    switch (enAudioFormat)
    {
        case HI_MPP_AUDIO_FORMAT_AAC:
            s32Ret = HI_MAPI_ADEC_AacInit();
            if (HI_SUCCESS != s32Ret)
            {
                AENV_TRACE(MPP_EN_ERR_LEVEL_ERROR, HI_MPP_MOD_AENC, "HI_MAPI_ADEC_AacInit  failed, s32Ret:%x\n",s32Ret);
                pthread_mutex_unlock(&m_AdecEncoderLock);
                return s32Ret;
            }
        ...
        default:
            AENV_TRACE(MPP_EN_ERR_LEVEL_ERROR, HI_MPP_MOD_AENC, "enAudioFormat:%d not support\n", enAudioFormat);
            pthread_mutex_unlock(&m_AdecEncoderLock);
            return HI_MAPI_ERR_ADEC_ILLEGAL_PARAM;
    }
