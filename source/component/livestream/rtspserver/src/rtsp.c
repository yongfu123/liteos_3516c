...
static inline int __connection_list_add(bufpool_handle con_pool, struct list_head_t *head,int fd, struct sockaddr_in addr, int server_id);
...
static int __bufpool_reset(void* buf)
{
	struct transfer_item_t *p = buf;

	memset(p, 0, sizeof(struct transfer_item_t));
...
static void __method_options(struct connection_item_t *p, rtsp_handle h)
{
	...
	send(p->client_fd, buff, strlen(buff), 0);
	#endif
...
static void __method_pause(struct connection_item_t *p, rtsp_handle h)
{
	...
	#else
	char buff[50] = {0};
	sprintf(buff, "RTSP/1.0 "__RESPONCE_STR_METHODNOTALLOWED "\r\n");
	send(p->client_fd, buff, strlen(buff), 0);
	#endif
...
static void __method_error(struct connection_item_t *p, rtsp_handle h)
{
	...
	#else
	char buff[50] = {0};
	sprintf(buff, "RTSP/1.0 "__RESPONCE_STR_SERVERERROR "\r\n");
	send(p->client_fd, buff, strlen(buff), 0);
	#endif
...
static void __method_play(struct connection_item_t *p, rtsp_handle h)
{
    ...
    p->rtp_timestamp = (int)(random())*(h->ctx);//rand_r(&h->ctx);
    p->rtp_seq = (int)(random())*(h->ctx);//rand_r(&h->ctx);
    p->rtcp_octet = 0; 
    p->rtcp_packet_cnt= 0; 
        ...
        if (con->parser_state == __PARSER_S_ERROR) {
            ...
            switch(con->method){
                ...
                case __METHOD_NONE: 
                    ...
                    DBG("con->con_state = %d, con->method=%d\r\n", con->con_state, con->method);
...
static int __connection_reset(void *v)
{
    ...
    static inline int
__connection_list_add(bufpool_handle con_pool, struct list_head_t *head,int fd, struct sockaddr_in addr, int server_id)
{
    ...
    //ASSERT((p->fp_tcp_write = fdopen(dup(fd),"w")), goto error);

    p->con_state = __CON_S_INIT;

    return list_add(head,&(p->list_entry));//将申请的缓存信息添加到缓冲列表中
error:
...
static inline int __accept_proc_sock(rtsp_handle h, int server_fd, struct sock_select_t *p_socks)
{
    ...
    if (FD_ISSET(server_fd, &p_socks->rfds) != 0) {
        ...
        ASSERT(__connection_list_add(h->con_pool,&h->con_list,fd, from_addr, server_fd) == SUCCESS,
                return FAILURE);
...
******************************************************************************/
Thr_Exit_Fun ThrExitFun = NULL;
int __Set_ThrExitCallBack(Thr_Exit_Fun fun)
{
	ASSERT(fun, return FALSE);
 ...
 *              PUBLIC FUNCTIONS
 ******************************************************************************/
void rtsp_finish(rtsp_handle rtsp_h)
{
    ...
    if (rtsp_h) {
        ...
        list_destroy(&rtsp_h->con_list);

        if (rtsp_h->pool) {

            gbl_set_quit(rtsp_h->pool->sharedp->gbl);

            ASSERT(threadpool_join(rtsp_h->pool) == SUCCESS, ERR("thread join with error\n"));

            bufpool_delete(rtsp_h->con_pool);
            bufpool_delete(rtsp_h->transfer_pool);

            mime_encoded_delete(rtsp_h->sprop_sps_b64);
            mime_encoded_delete(rtsp_h->sprop_sps_b16);
            mime_encoded_delete(rtsp_h->sprop_pps_b64);

            threadpool_delete(rtsp_h->pool);
        }

        pthread_mutex_destroy(&rtsp_h->mutex);
...
rtsp_handle rtsp_create(unsigned char max_con, int priority, Thr_Exit_Fun fun)
{
    rtsp_handle       nh = NULL;
	...
	ASSERT(__Set_ThrExitCallBack(fun) == SUCCESS, 
			goto error);
