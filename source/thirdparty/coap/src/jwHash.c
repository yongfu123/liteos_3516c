...
// Lookup int - keyed by int
HASHRESULT get_int_by_int( jwHashTable *table, long int key, int *value )
{
	// compute hash on key
	size_t hash = hashInt(key) % table->buckets;
	HASH_DEBUG("fetching %d -> ?? hash: %d\n",key,hash);
...
HASHRESULT add_int_by_ptr( jwHashTable *table, void *key, long int value )
{
	// compute hash on key
	size_t hash = hashInt((long int)key) % table->buckets;
	HASH_DEBUG("adding %d -> %d hash: %d\n",key,value,hash);
	...
	while(entry!=0)
	{
		HASH_DEBUG("checking entry: %x\n",entry);
		// check for already indexed
		if(entry->key.ptrValue==key && value==entry->value.intValue)
			return HASHALREADYADDED;
		// check for replacing entry
		if(entry->key.ptrValue==key && value!=entry->value.intValue)
		{
...
HASHRESULT get_int_by_ptr( jwHashTable *table, void *key, int *value )
{
	// compute hash on key
	size_t hash = hashInt((long int)key) % table->buckets;
	HASH_DEBUG("fetching %d -> ?? hash: %d\n",key,hash);
