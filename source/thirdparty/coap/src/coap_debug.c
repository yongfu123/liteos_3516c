...
coap_print_addr(const struct coap_address_t *addr, unsigned char *buf, size_t len) {
  ...
  unsigned char *p = buf;
#ifndef WITH_LWIP
  int sa_family = addr->addr.sa.sa_family;
#else
  int sa_family = AF_INET; //our lwip only support ipv4
#endif
  ...
  if (inet_ntop(sa_family, addrptr, (char *)p, len) == 0) {
...
coap_show_pdu(const coap_pdu_t *pdu) {
  ...
  while ((option = coap_option_next(&opt_iter))) {
	...
	debug(" %s:%.*s", msg_option_string(opt_iter.type),
	    (int)buf_len, buf);
  }
