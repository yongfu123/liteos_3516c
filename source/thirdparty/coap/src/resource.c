...
void
coap_print_json(const coap_resource_t *resource,
                cJSON *object)
{
	coap_attr_t *attr;
	static char temp_buf[128];
	static char temp_buf2[128];
	assert(!(256 <= resource->uri.length));
	memcpy(temp_buf, resource->uri.s, resource->uri.length);
	temp_buf[resource->uri.length] = '\0';
	cJSON_AddItemToObject(object, "href", cJSON_CreateString(temp_buf));
            ...
            attr = list_item_next(attr))
    {
		...
		if (attr->value.s) {
			memcpy(temp_buf, attr->value.s, attr->value.length);
			temp_buf[attr->value.length] = '\0';
			memcpy(temp_buf2, attr->name.s, attr->name.length);
			temp_buf2[attr->name.length] = '\0';
			cJSON_AddItemToObject(object, temp_buf2, cJSON_CreateString(temp_buf));
		}
    }
    if (resource->observable)
    {
        cJSON_AddItemToObject(object, "obs", cJSON_CreateString("true"));
    }
...
coap_print_wellknown(coap_context_t *context, unsigned char *buf, size_t *buflen,
		size_t offset,int media_type, 
		coap_opt_t *query_filter __attribute__ ((unused))) {
#else /* not a GCC */
coap_print_status_t
coap_print_wellknown(coap_context_t *context, unsigned char *buf, size_t *buflen,
		size_t offset, int media_type, coap_opt_t *query_filter) {
#endif /* GCC */
  coap_resource_t *r;
  unsigned char *p = buf;
...
#if !defined(WITH_CONTIKI)&&!defined(WITH_LITEOS_UIP)  
...
#define MATCH_SUBSTRING 0x04
  ...
  cJSON_AddItemToObject(root,"services",array = cJSON_CreateArray());
    ...
    cJSON_Delete(root);
    if (json_output) output_len = strlen(json_output);

    if (*buflen < (output_len - offset))
    {
        memcpy(buf,json_output+offset, *buflen);
        result = *buflen;
        result |= COAP_PRINT_STATUS_TRUNC;
    }
    else
    {
        memcpy(buf,json_output+offset, output_len - offset);
...
coap_free_resource(coap_resource_t *resource) {
  ...
  //LL_FOREACH_SAFE(resource->subscribers, obs, otmp) COAP_FREE_TYPE(subscription, obs); //TODOO
...
coap_add_observer(coap_resource_t *resource, 
   ...
   * another one. */
  //s = COAP_MALLOC_TYPE(subscription);//TODOO
...
coap_delete_observer(coap_resource_t *resource, const coap_address_t *observer,
  ...
  if (resource->subscribers && s) {
    ...
    //COAP_FREE_TYPE(subscription,s);//TODOO
...
#define INET6_ADDRSTRLEN 40
	...
	//COAP_FREE_TYPE(subscription, obs); //TODOO
