...
#ifdef MALLOC_STATISTICS
static jwHashTable *s_table = NULL;
static int s_ram_malloc = 0;
#endif
...
#ifdef MALLOC_STATISTICS
    if (NULL == s_table) {
        s_table = create_hash(16);
    }
    void *p = malloc(size);
    add_int_by_ptr(s_table, p, size);
    s_ram_malloc += size;
    printf("type:%d, malloc %d bytes, now total %d bytes.\n", type, size, s_ram_malloc);
    return p;
#else
  return malloc(size);
#endif
...
#ifdef MALLOC_STATISTICS
    int size;
    get_int_by_ptr(s_table, p, &size);
    s_ram_malloc -= size;
    printf("type:%d, free %d bytes, now total %d bytes.\n", type, size, s_ram_malloc);
#endif
  free(p);
}
