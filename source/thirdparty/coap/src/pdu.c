...
#ifdef WITH_LWIP
coap_del_nodecallback coap_del_nodecb = NULL;
/**************************************************
register function to tell upcall that node does del
***************************************************/
void coap_del_nodecall(void *fun)
{
	if ( fun ) {
		coap_del_nodecb = fun;
	}
