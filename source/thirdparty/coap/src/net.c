...
coap_new_context(
  ...
  /* initialize observe value, make it non-zero */
  c->observe = 2;
...
coap_transaction_id(const coap_address_t *peer, const coap_pdu_t *pdu, 
  ...
  *id = (((h[0] << 8) | h[1]) ^ ((h[2] << 8) | h[3])) & INT_MAX;//INT_MAX = 0x7fffffff
}
...
coap_send_impl(coap_context_t *context,
  ...
  if(p->payload == pdu) printf("PDU not restored \n");
    
  //coap_show_pdu(pdu);

  //pdu->hdr = p->payload;//p->payload + sizeof(coap_pdu_t);
...
coap_send_confirmed(coap_context_t *context, 
  ...
  node = coap_new_node();
  if (!node) {
    printf("coap_send_confirmed: insufficient memory\n");
    return COAP_INVALID_TID;
  }

  node->id = coap_send_impl(context, local_interface, dst, pdu);
  if (COAP_INVALID_TID == node->id) {
    printf("coap_send_confirmed: error sending pdu\n");
    coap_free_node(node);
    return COAP_INVALID_TID;
  }
...
get_wkc_len(coap_context_t *context, coap_opt_t *query_filter) {
  ...
  if (coap_print_wellknown(context, buf, &len, UINT_MAX, 0, query_filter)
...
coap_wellknown_response(coap_context_t *context, coap_pdu_t *request) {
  ...
  *resp->data = COAP_PAYLOAD_START;
  ...
  result = coap_print_wellknown(context, resp->data, &len, offset, 0, query_filter);
