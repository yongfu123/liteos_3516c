...
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
static struct himedia_device s_stSil902xDevice;
#endif
...
-------------------------------------------------------------------*/
#ifdef __HuaweiLite__
struct i2c_client sil_client_obj;
struct i2c_client * hi_sil_i2c_client_init(void)
{
	sil_client_obj.addr = TX_SLAVE_ADDR;//sensorμ?I2Cμ??·?￡
	sil_client_obj.flags = 0x00;
	sil_client_obj.pinfo = &hi_i2c_data[2];//ê1ó?μ?ê?I2C0 ?￡
	hi_i2c_hw_init(sil_client_obj.pinfo);
	return &sil_client_obj;
}
#endif
#ifdef __HuaweiLite__
int i2c_client_init(void)
{
    sil_client = hi_sil_i2c_client_init();
    return 0;
}
#else
static int i2c_client_init(void)
{
    ...
    //ASIC i2c3
    i2c_adap   = i2c_get_adapter(SIL9024_I2C_ADAPTER_ID);
    ...
    unsigned int reg_addr_num, unsigned int data_byte_num)
{
    ...
    memcpy(&client, sil_client, sizeof(struct i2c_client));
...
unsigned char sil_i2c_read_byte(unsigned char dev_addr, unsigned char reg_addr)
{
    return hi_i2c_read(dev_addr, reg_addr, 1, 1);
}
int sil_i2c_write_byte(unsigned char dev_addr, unsigned char reg_addr, unsigned char data)
{
    return hi_i2c_write(dev_addr, reg_addr, 1, data, 1);
}

#else
unsigned char sil_i2c_read_byte(unsigned char devaddress, unsigned char address)
{
...
int sil_i2c_write_byte(unsigned char devaddress, unsigned char address, unsigned char data)
{
    int ret;
    unsigned char buf[2];
    struct i2c_client* client = sil_client;
...
#define  I2CReadByte   sil_i2c_read_byte
#define  I2CWriteByte  sil_i2c_write_byte
/*----------------------------------------------------------------------------
...
// Globals: none
//------------------------------------------------------------------------------
void siHdmiTx_TPI_Poll (unsigned long data)
{
...
#endif
    {
        ...
        if (InterruptStatus & HOT_PLUG_EVENT)
        {
            ...
            do
            {
                ...
                TPI_DEBUG_PRINT (("InterruptStatus: 0x%x\n", InterruptStatus));
            }
...
//------------------------------------------------------------------------------
#ifdef __HuaweiLite__
static int sil902x_open(struct file* file);
static int sil902x_close(struct file* file);
#else
...
static long sil902x_ioctl(struct file* file, unsigned int cmd, unsigned long arg);
...
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
static int sil902x_device_init_restore(void);

static int sil902x_freeze(struct himedia_device* pdev)
{
    printk(KERN_ALERT "%s  %d\n", __FUNCTION__, __LINE__);
    return 0;
}

static int sil902x_restore(struct himedia_device* pdev)
{
    printk(KERN_ALERT "%s  %d\n", __FUNCTION__, __LINE__);
    sil902x_device_init_restore();
...
#ifdef __HuaweiLite__
const static struct file_operations_vfs sil902x_fops =
{
    .open = sil902x_open,
    .close = sil902x_close,
    .ioctl = sil902x_ioctl
...
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
struct himedia_ops stSil902xDrvOps =
{
    .pm_freeze = sil902x_freeze,
    .pm_restore  = sil902x_restore
};
#else
...
#ifndef __HuaweiLite__
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
                    himedia_unregister(&s_stSil902xDevice);
#else
...
#ifdef __HuaweiLite__
int sil902x_init(void *pArgs)
{
    unsigned int ret = 0;
    SIL9024_MODULE_PARAMS_S *pstSilModuleParam = (SIL9024_MODULE_PARAMS_S *)pArgs;
    if(NULL != pstSilModuleParam)
    {
        norm = pstSilModuleParam->norm;
    }

	ret = register_driver("/dev/sil9024", &sil902x_fops, 0666, 0);
    if (ret)
    {
    ...
    if (sil902x_device_init() < 0)
    {
        unregister_driver("/dev/sil9024");
        return -1;
    }
    return ret;
}
void sil902x_exit(void)
{
    del_timer_sync(&sil902x_timer);
    unregister_driver("/dev/sil9024");
    DPRINTK(1, "deregister sil9024!");
    return;
}
#else
...
#ifdef CONFIG_HISI_SNAPSHOT_BOOT
    snprintf(s_stSil902xDevice.devfs_name, sizeof(s_stSil902xDevice.devfs_name), DEV_NAME);

    s_stSil902xDevice.minor  = HIMEDIA_DYNAMIC_MINOR;
    s_stSil902xDevice.fops   = &sil902x_fops;
    s_stSil902xDevice.drvops = &stSil902xDrvOps;
    s_stSil902xDevice.owner  = THIS_MODULE;

    ret = himedia_register(&s_stSil902xDevice);
    if (ret)
    {
        DPRINTK(0, "could not register sil902x device");
        return -1;
...
#if 0
    // 2102/10/17 by lkf72840   修改以下三个寄存器的值  解决 3518 上9022 无输出和偏红的问题
    I2CWriteByte(0x72, 0x63, 0x40);
    I2CWriteByte(0x72, 0x64, 0x43);
    I2CWriteByte(0x72, 0x19, 0x00);
