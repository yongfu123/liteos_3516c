...
static int check_lane_id(input_mode_t input_mode, short* p_lane_id)
{
	...
	printf("-------%d,%d,%d,%d,%d,%d,%d,%d\n", p_lane_id[0], p_lane_id[1],p_lane_id[2], p_lane_id[3], p_lane_id[4], p_lane_id[5], p_lane_id[6], p_lane_id[7]);
...
static inline int is_dev_cfged(COMBO_DEV devno)
{
    ...
    if (g_lane_info.link_num <= 0 || g_lane_info.total_lane_num <= 0)
    {
        return HI_FALSE;
    }
...
static int mipi_set_phy_cmvmode(COMBO_DEV devno, phy_cmv_e cmv_mode)
{
    ...
    for (i = 0; i < g_lane_info.link_num; i++)
    {
        link_id = g_lane_info.link_arr[i].link_idx;
...
void mipi_drv_reset_sensor(COMBO_DEV dev)
{
    ...
    #else
    unsigned int sensor_reset = (unsigned int)OSAL_IO_ADDRESS(0x12010040);
...
void mipi_drv_unreset_sensor(COMBO_DEV dev)
{
	    ...
	    mipi_write_reg32(sensor_reg, reset << 20, 1 << 20);
	#else
	    unsigned int sensor_reg = (unsigned int)OSAL_IO_ADDRESS(0x12010040);
...
#ifndef HI_FPGA
	return;
	unsigned int mipi_reset  = (unsigned int)OSAL_IO_ADDRESS(0x1201003c);
...
static long hi_mipi_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
	...
	osal_printk("cmd = %x, HI_MIPI_SET_DEV_ATTR = %x\r\n", cmd, HI_MIPI_SET_DEV_ATTR);
    switch (cmd)
    {
		...
		{
			phy_cmv_t *pstphy_cmv;

			pstphy_cmv = (phy_cmv_t *)argp;
			ret = mipi_set_phy_cmvmode(pstphy_cmv->devno, pstphy_cmv->cmv_mode);
			break;
		}
