...
unsigned int ir_irq = HIIR_DEVICE_IRQ_NO;

/* define IR IO */
void* ir_reg_base = NULL;
//#define IR_REG_BASE (OSAL_IO_ADDRESS(0x20070000))
...
static long hiir_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    switch(cmd)
    {
        ...
        case IR_IOC_SET_LEADS:
            hiir_dev.dev_parm.leads_min = p[0];
            hiir_dev.dev_parm.leads_max = p[1];
        ...
        case IR_IOC_SET_LEADE:
            hiir_dev.dev_parm.leade_min = p[0];
            hiir_dev.dev_parm.leade_max = p[1];
        ...
        case IR_IOC_SET_SLEADE:
            hiir_dev.dev_parm.sleade_min = p[0];
            hiir_dev.dev_parm.sleade_max = p[1];
        ...
        case IR_IOC_SET_CNT0_B:
            hiir_dev.dev_parm.cnt0_b_min = p[0];
            hiir_dev.dev_parm.cnt0_b_max = p[1];
        ...
        case IR_IOC_SET_CNT1_B:
            hiir_dev.dev_parm.cnt1_b_min = p[0];
            hiir_dev.dev_parm.cnt1_b_max = p[1];
...
retry :
    ...
    if((hiir_dev.head)==(hiir_dev.tail))
    {
        ...
        res = osal_wait_event(&(hiir_dev.irkey_wait),(hiir_dev.head != hiir_dev.tail));
        if (0 != res)
        {
            return res;
        }
...
int hiir_init(void)
{
    ...
    if (NULL==ir_reg_base)
    {
        ir_reg_base = OSAL_IO_ADDRESS(0x20070000);
    }
